...
# effects.
  for(site in 1:nsite){
    for(yr in 1:nyear){
      ...
      psi[ site, yr, 2 ] <- exp(a0 + inprod(a, acovs[ site, ]) +  
                                aseason[ sea_vec[yr] ] + ayear[ year_vec[yr] ])
      # coyote with mange occupancy linear predictor
      psi[ site, yr, 3 ] <- exp(a0 + inprod(a, acovs[ site, ]) +  
                                aseason[ sea_vec[yr] ] + ayear[ year_vec[yr] ] +
                                b0 + inprod(b, bcovs[ site, yr, ]) +  
                                bseason[ sea_vec[yr] ] + byear[ year_vec[yr] ])
    }
  } 
#
# psi tpm - CLOSE 
...
byear[1] <- 0
for(yr_prior in 1:3){
  ayearpar[yr_prior] ~ dlogis(0, 1) 
  byearpar[yr_prior] ~ dlogis(0, 1) 
}
#
for(fill_yr in 2:4){
  ayear[fill_yr] <- ayearpar[fill_yr-1]
  byear[fill_yr] <- byearpar[fill_yr-1]
}
# priors - CLOSE
# priors - CLOSE
# priors - CLOSE
