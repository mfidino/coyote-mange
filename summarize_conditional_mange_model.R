...
sig_at_95 <- which(rowSums(sign(base_results[1:14,])) %in% c(3, -3))
...
X <- expand.grid(tmp1, tmp2)

# drop stuff that

X <- as.matrix(cbind(1, X, X[,1] * X[,2]))
...
res_mat <- matrix(dfo[,5] * df[,5], nrow = length(tmp1), ncol =length(tmp2))

# determine where 1) coyotes were detected and 2) coyotes with mange were detected

# coyote present, plus an indicator if mange was detected.
coy_p <- coy %>% group_by(site, surveyid) %>% 
...
coy_p <- coy_p %>% group_by(site) %>% 
  summarise(sdet = length(surveyid),
            mdet = sum(mange))

# join coy_p with the urbanization covariates
coy_p <- inner_join(coy_p, urb_mat, by = 'site')

# make new columns that denote shape and color of these points
coy_p$color <- ifelse(coy_p$mdet > 0, "gray20", "gray")
coy_p$pch <- ifelse(coy_p$mdet > 0, 24, 21)

purple_pal <- colorRampPalette(c('white', '#C083F8', '#8F32CE'))
windows(6,5)
tiff('coyote_mange_plot.tiff', height = 5, width = 6, 
     units = 'in', res = 600, compression = "lzw")
par(mar= c(4,5.5,1,1))
plot(1~1, type = 'n', ylim = range(tmp2), xlim = range(tmp1), bty = 'n',
     xaxt = 'n', yaxt = 'n', xlab = "", ylab = "", xaxs = 'i', yaxs = 'i')
image(tmp1, tmp2, res_mat, col = purple_pal(100), xlab = "URB1", ylab = "URB2",
      las = 1, xaxt = 'n', yaxt = 'n', add = TRUE)
axis(1, seq(min(tmp1), max(tmp1), 1), labels = FALSE, tck = -0.035)
axis(1, seq(min(tmp1), max(tmp1), 0.5), labels = FALSE, tck = -0.035/2)
mtext(text = sprintf("%.1f", seq(min(tmp1), max(tmp1), 1)), 1, line = 0.9,
      at = seq(min(tmp1), max(tmp1), 1), cex = 1.2)
axis(2, seq(min(tmp2), max(tmp2), 0.5), labels = FALSE, tck = -0.035, cex = 1.1)
axis(2, seq(min(tmp2), max(tmp2), 0.25), labels = FALSE, tck = -0.035/2, cex = 1.1)
mtext(text = sprintf("%.1f", seq(min(tmp2), max(tmp2), 0.5)), 2, line = 0.9,
      at = seq(min(tmp2), max(tmp2), 0.5), cex = 1.2, las = 1)
mtext(text = "URB1",1, line = 2.5, at = mean(tmp1), cex = 1.5)
mtext(text = "URB2",2, line = 3.3, at = mean(tmp2), cex = 1.5)
contour(tmp1, tmp2, res_mat, add = TRUE, nlevels = 5, lwd = 2, labcex = 1.2)
points(coy_p$urb2 ~ coy_p$urb1, bg = coy_p$color, pch = coy_p$pch, cex = 1.2)

legend('bottomleft', bg = 'white', pt.bg = c('gray20', 'gray'), pch = c(24,21), 
       legend = c('with mange', 'without mange'), title = 'Coyote detected...',
       pt.cex = 1.2, cex = 1.1)
dev.off()
...
# a matrix of the mange results
res_mat <- matrix(df[,5], nrow = length(tmp1), ncol =length(tmp2))
...
# join coy_p with the urbanization covariates
coy_p <- right_join(coy_p, urb_mat, by = 'site')

# make new columns that denote shape and color of these points
coy_p$color <- ifelse(!is.na(coy_p$mdet), "gray20", "gray")
coy_p$pch <- ifelse(!is.na(coy_p$mdet), 24, 21)


windows(6,5)
tiff('coyote_plot.tiff', height = 5, width = 6, 
     units = 'in', res = 600, compression = "lzw")
par(mar= c(4,5.5,1,1))
plot(1~1, type = 'n', ylim = range(tmp2), xlim = range(tmp1), bty = 'n',
     xaxt = 'n', yaxt = 'n', xlab = "", ylab = "", xaxs = 'i', yaxs = 'i')
