  ...
  for(site in 1:nsite){
    for(yr in 1:nyear){
      for(surv in 1:nsurvey){
        y[ site, yr, surv ] ~ dcat(lambda[ site, yr,surv,1:nstate, z[site, yr] ])
      } 
    }    
  } 
#
  ...
  for(site in 1:nsite){
    for(yr in 1:nyear){
      pr_occ_coyote[site, yr] <- ilogit(a0 + inprod(a, acovs[ site, ]) +  
                                aseason[ sea_vec[yr] ] )
      pr_occ_mcoyote[site, yr] <- ilogit(b0 + inprod(b, bcovs[ site, ]) +  
                                    bseason[ sea_vec[yr] ] )
...
#
  for(site in 1:nsite){
    ...
    pr_coyote[site, yr] <- ilogit( d0 + inprod(d, dcovs[site, ]) + 
                                    dseason[ sea_vec[yr] ] )
    #pr_missed_mange[site, yr] <- ilogit(g0 + inprod(g, gcovs[yr,]))
    for(week in 1:nweek){
      # This switches between the probability of detecting
      #  mange conditional on having images vs the probability of detecting mange
      #  when we have 0 images, which we assume is zero.
      pr_mangy_coyote[site,yr,week] <- hp[site,yr,week] * 
        (1 - ( exp(sum(log((1-mange_prob[(mmin[site,yr,week]):(mmax[site,yr,week])])))))) +
        (1 - hp[site,yr,week]) * 0
      # The probability matrix. TS = True State. OS = Observed state
      # TS = no coyote = 1
...
#
  for(photo in 1:nphoto){
    ...
    mange_prob[photo] <- ifelse(mange_signs_present[photo] == 1, 1, mange_mu[photo])
  }
# 
...
#
# Pr(detecting mange in an image)
f0 ~ dlogis(0, 1)
for(f_covs in 1:4){
  f[f_covs] ~ dlogis(0, 1)
}
#
# Pr(TS = mange but we did not detect coyote)
#
g0 ~ dlogis(0, 1)
g ~ dlogis(0, 1)
# Seasonal effects
...
for(fillsea in 2:4){
  ...
  gseason[fillsea] <- gseasonpar[fillsea-1]
}
#
# priors - CLOSE
# priors - CLOSE
